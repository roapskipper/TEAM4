...
public class BiddingService {
    ...
    // Map to store locks per auction ID to ensure thread-safety without blocking
    // other auctions
    ...
    public BidTransaction placeBid(String auctionId, String bidderId, BigDecimal bidAmount) {
        ...
        lock.lock();
        try {
            // 1. Fetch current auction to ensure we have the latest state (avoiding lost
            // updates)
            ...
            // 2. Apply new bid. This includes checking if the bid is valid/higher than the
            ...
            // 6. Anti-sniping logic: if bid is placed within the last 30 seconds, extend
            // time by 60 seconds
